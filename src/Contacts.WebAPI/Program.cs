...
app.MapGet("/api/contacts", async ([FromQuery]string? q, ApplicationDbContext db) =>
{
    ...
    } else
    {
        contacts = await db.Contacts.Where(x =>
            !string.IsNullOrWhiteSpace(x.Name) && x.Name.ToLower().IndexOf(q) >= 0 ||
            !string.IsNullOrWhiteSpace(x.Email) && x.Email.ToLower().IndexOf(q) >= 0 ||
            !string.IsNullOrWhiteSpace(x.Phone) && x.Phone.ToLower().IndexOf(q) >= 0 ||
            !string.IsNullOrWhiteSpace(x.Address) && x.Address.ToLower().IndexOf(q) >= 0).ToListAsync();
    }
