    ...
    public class ContactWebApiRepository : IContactRepository
    {
        ...
        public async Task<CoreBusiness.Contact> GetContactByIdAsync(int contactId)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                contact = JsonSerializer.Deserialize<CoreBusiness.Contact>(content);
            }
            return contact;
        }

        public async Task<List<CoreBusiness.Contact>> GetContactsAsync(string query)
        {
            ...
            {
                uri = new Uri($"{Constants.WebApiBaseUrl}/contacts?q={query}");
            }
            var response = await _client.GetAsync(uri);
            if (response.IsSuccessStatusCode)
            {
